...
router.post('/public-process-followups', async (req, res) => {
  // Simula um usuário logado para o banco de dados registrar a atividade
...
router.post('/bulk', async (req, res) => {
  try {
    ...
    // 🏆 RESPOSTA ANTECIPADA (EARLY RESPONSE) — Vital para evitar erro de JSON na captura em massa
    res.status(201).json({ 
      sucesso: true, 
      message: `${data.length} leads capturados no CRM com sucesso`, 
      leads: data 
    });

    // 🕒 TAREFAS EM BACKGROUND (IA / n8n)
...
router.post('/', async (req, res) => {
  try {
    ...
    // 🏆 RESPOSTA ANTECIPADA (EARLY RESPONSE) - Envia sucesso para a extensão na hora!
    res.status(201).json({ sucesso: true, message: 'Lead capturado com sucesso', lead });

    // 🕒 TAREFAS EM BACKGROUND (IA / n8n / WHATSAPP) - Sem travar a extensão
    try {
      // 1. Notificar n8n
      ...
      if (lead && (headline || about)) {
        enriquecerLeadComIA(lead).catch(e => console.error('Erro IA Background:', e.message));
      }

      // 3. Jornada de 96 dias - Follow-up Automático
      agendarFollowUps(lead.id, req.user.userId).catch(e => console.error('Erro FollowUp Background:', e.message));

    } catch (bgError) {
      console.error('Erro nas tarefas de background do lead:', bgError.message);
    }
  } catch (err) { 
    console.error('Erro fatal na criação de lead:', err.message);
    if (!res.headersSent) res.status(500).json({ error: err.message }); 
  }
});
