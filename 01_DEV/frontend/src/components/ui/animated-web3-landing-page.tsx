...
export function Web3HeroAnimated() {
  ...
  return (
    <>
      ...
      <section className="relative isolate h-screen overflow-hidden bg-black text-white font-sans">
        {/* ================== BACKGROUND ================== */}
        <div
          aria-hidden
        ...
        <div className="relative z-10 mx-auto mt-10 w-full max-w-6xl px-6 pb-24">
          <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-6 opacity-70">
            {["git","npm","Lucidchart","wrike","jquery","openstack","servicenow","Paysafe"].map((brand) => (
              <div key={brand} className="text-xs uppercase tracking-wider text-white/70 hover:text-white cursor-default transition-colors">{brand}</div>
            ))}
          </div>
        </div>
        ...
        {/* ================== FOREGROUND ================== */}
        <div
          className="pointer-events-none absolute bottom-[128px] left-1/2 z-0 h-36 w-28 -translate-x-1/2 rounded-md bg-gradient-to-b from-white/75 via-rose-100/60 to-transparent blur-xl opacity-50"
